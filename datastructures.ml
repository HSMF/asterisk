...
(* I got this from the ETH Compiler Design course but they probably got it from
   somewhere too so idk where the credit goes to *)
...
module MakeMap (Ord : OrdPrintT) : MapS with type key = Ord.t = struct
  ...
  let diff_keys cmp_v m n =
    ...
    let has_binding_or_add m k v l =
      try if cmp_v v @@ find k m == 0 then l else S.add k l with
      | Not_found -> S.add k l
    in
    S.empty
